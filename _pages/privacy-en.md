...
[한국어 정본 보기 (Korean authoritative version) →](/privacy/)
...
This document is a reference translation provided for convenience. The Korean version (개인정보처리방침) is the authoritative and legally binding version. In the event of any inconsistency between the Korean version and this English translation, the Korean version shall prevail.
...
- When using Sign in with Apple, you may use Apple's "Hide My Email" feature, in which case a relay email address generated by Apple is collected instead of your actual email address.
- The social account's profile photo is not used by the Service, and the Operator does not collect phone numbers, dates of birth, location information, or advertising identifiers.
...
3. Upon withdrawal, traces of the withdrawn member remaining in other users' content (likes, follow relationships, reviewer aggregates, etc.) are also deleted. If any information is not deleted immediately due to a system error or similar cause, the Operator will delete it as soon as it becomes aware of it. However, information related to the withdrawn member contained in other users' data (e.g., identifiers stored in other users' block lists, or identifiers remaining as references in comments written by other users) may remain as those users' data; after account deletion, such identifiers alone cannot identify a specific individual.
...
4. The Operator does not collect advertising identifiers (ADID/IDFA) and does not use advertising or behavioral analytics SDKs.
...
This Privacy Policy takes effect on July 4, 2026.
